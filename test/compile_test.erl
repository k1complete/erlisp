...
compile_test() ->
    {ok, _ModuleName, _Binary} = erlisp_compile:file("test/testdata/c1.elisp", []),
    ?assertEqual(2 , c1:erun(1)).


compile_doctest_test() ->
    {ok, Module, Binary, _Ast} = erlisp_compile:file_ast("test/testdata/doctest.elisp", []),
...
compile_with_macro_test() ->
    {ok, Module, _Binary, _Ast} = erlisp_compile:file_ast("test/testdata/macrotest.elisp", []),
...
compile_with_macrofunccall_test() ->
    {ok, Module, _Binary, Ast} = erlisp_compile:file_ast("test/testdata/macrofunccall.elisp", []),
